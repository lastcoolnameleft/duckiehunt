...
def upload_photo(duck_location_id, image_path, duck_id, duck_name, comments):
    """
    Upload a photo to the configured photo provider and attach it to the sighting.

    Args:
        duck_location_id: PK of the DuckLocation to attach the photo to
    ...
    try:
        ...
        logger.error("upload_photo: DuckLocation %s not found", duck_location_id)
        return
    ...
    try:
        ...
        photo = DuckLocationPhoto(
            duck_location=duck_location,
            flickr_photo_id=result['id'],
            flickr_thumbnail_url=result['thumbnail_url'],
        )
        photo.save()
        logger.info("Photo uploaded for duck #%s (location %s)", duck_id, duck_location_id)
    except Exception:
        logger.exception("Photo upload failed for duck #%s (location %s)", duck_id, duck_location_id)
